...
> 审计日期：2026-09-05
> 基线提交：`0bfe32b`（`my-custom`）
> 状态：`2026.09.05-custom.5` 候选与精确暂存审计完成；等待提交和同步远端。

> 当前白名单：`88` 个路径；补齐图片生成内容包覆盖、主壳层备份弹窗、统一缓存版本及相关回归文件。
...
- [x] 已统一 `VERSION`、`APP_VERSION`、`static/update-notes.json` 与静态缓存参数为 `2026.09.05-custom.5`。
- [x] 已只用精确路径暂存；`API/.env`、`data/asset_library.json`、案例媒体和其它本机文件均未进入暂存差异。
